...
compile_app() {
    ...
    # Step 4: Fine-grained lowering (target-specific)
    if [ "$TARGET_BACKEND" = "bf2" ]; then
        # BF2: vDRMT → bf2drmt (DOCA 2.2)
        print_step "Step 4a: Fine-Grained Lowering (vDRMT → bf2drmt)"
        DOCA_TEMPLATE="$ROOT_DIR/templates/doca-2.2.toml" \
            "$NUTCRACKER_OPT" --vdrmt-to-bf2drmt 2>&1
    else
        # BF3 (default): vDRMT → bf3drmt + vDPP → bf3dpa
    ...
    # Step 5.5: Emit handler C code (mapping-aware) — BF3 only
    # BF2's doca_flow_pipeline.c is already generated by --vdrmt-to-bf2drmt.
    if [ "${TARGET_BACKEND:-bf3}" != "bf2" ]; then
...
main() {
    ...
    case "$command" in
        ...
        compile)
            if [ $# -eq 0 ]; then
                print_error "Application name required"
                echo "Usage: $0 compile <app_name> [--target bf2|bf3]"
                exit 1
            fi
            APP_NAME="$1"; shift
            TARGET_BACKEND="bf3"  # default
            while [ $# -gt 0 ]; do
                case "$1" in
                    --target) TARGET_BACKEND="$2"; shift 2 ;;
                    *) shift ;;
                esac
            done
