...
std::string Egglog::eggifyType(mlir::Type type) {
    ...
    // llvm::outs() << "Eggifying type: " << type << ", type string: " << typeStr << "\n";
    if (type.isF16()) {
        ss << "(F16)";
    } else if (type.isF32()) {
        ss << "(F32)";
...
mlir::Attribute Egglog::parseAttribute(const std::string& attrStr) {
    ...
    if (attrType == "IntegerAttr") {
        ...
        std::string symbolStr = unwrap(split[1], '"');
        // Handle nested symbol references like "Register::read"
        if (symbolStr.find("::") != std::string::npos) {
            size_t pos = symbolStr.find("::");
            std::string rootSymbol = symbolStr.substr(0, pos);
            std::string nestedSymbol = symbolStr.substr(pos + 2);
            
            auto nestedRef = mlir::SymbolRefAttr::get(&context, nestedSymbol);
            return mlir::SymbolRefAttr::get(&context, rootSymbol, {nestedRef});
        } else {
            return mlir::SymbolRefAttr::get(&context, symbolStr);
        }
...
std::string Egglog::eggifyAttribute(mlir::Attribute attr) {
    ...
    if (typeId == mlir::TypeID::get<mlir::IntegerAttr>()) {  // (IntegerAttr <int> <type>)
        ...
        if (elementType.isa<mlir::IntegerType>()) {
            mlir::IntegerType intType = elementType.cast<mlir::IntegerType>();
        ...
        mlir::SymbolRefAttr symbolRefAttr = attr.cast<mlir::SymbolRefAttr>();
        std::string value = symbolRefAttr.getRootReference().str();
        llvm::outs() << "Eggifying SymbolRefAttr: " << value << "\n";
        
        // Handle nested references like "Register::read"
        for (auto nested : symbolRefAttr.getNestedReferences()) {
            llvm::outs() << "Nested reference: " << nested.getRootReference().str() << "\n";
            value += "::" + nested.getRootReference().str();
        }
...
mlir::Operation* Egglog::parseOperation(const std::string& newOpStr, mlir::OpBuilder& builder) {
    ...
    // Derive the MLIR op name by replacing only the first '_' in fullName with '.'.
    // e.g. "bf3drmt_struct_extract_ref" -> "bf3drmt.struct_extract_ref"
    // This preserves multi-segment op names that use '_' within the op name.
    std::string mlirOpName = egglogOpDef.fullName;
    {
        size_t pos = mlirOpName.find('_');
        if (pos != std::string::npos) mlirOpName[pos] = '.';
    }
...
EggifiedOp* Egglog::eggifyOperation(mlir::Operation* op) {
    ...
    // // Dump operands & results
    // llvm::outs() << " - Num operands: " << op->getNumOperands() << "\n";
    // llvm::outs() << " - Num results:  " << op->getNumResults() << "\n";
    // llvm::outs() << " - Num regions:  " << op->getNumRegions() << "\n";

    // // Dump all attributes first
    // llvm::outs() << " - Attributes (" << op->getAttrs().size() << "):\n";
    ...
    EggifiedOp* foundEggifiedOp = findEggifiedOp(op);
    if (foundEggifiedOp != nullptr) {
        llvm::outs() << "Already eggified -> id=" << foundEggifiedOp->getPrintId() << "\n";
        return foundEggifiedOp;
    }
    ...
    // Check if op with operand-count suffix exists
    if (!isSupported) {
        std::string opNameWithNumOperands = opName + "." + std::to_string(op->getNumOperands());
        isSupported = supportedEgglogOps.find(opNameWithNumOperands) != supportedEgglogOps.end();
        if (!isSupported) {
            llvm::outs() << "!! Unsupported op '" << opName << "' and '" 
                         << opNameWithNumOperands << "'\n";
            return eggifyOpaqueOperation(op);
        }
        opName = opNameWithNumOperands;
    }
    ...
    EgglogOpDef egglogOpDef = supportedEgglogOps.at(opName);
    // llvm::outs() << "Egglog op: " << egglogOpDef.fullName
    //              << " expects (Operands=" << egglogOpDef.nOperands
    //              << ", Results=" << egglogOpDef.nResults
    //              << ", Attrs=" << egglogOpDef.nAttributes
    //              << ", Regions=" << egglogOpDef.nRegions << ")\n";

    // Check operand mismatch
    if (egglogOpDef.nOperands != op->getNumOperands()) {
        // llvm::outs() << "!! Operand mismatch: MLIR has " << op->getNumOperands()
        //              << ", Egglog expects " << egglogOpDef.nOperands << "\n";
        return eggifyOpaqueOperation(op);
    }

    // Check result mismatch
    if (egglogOpDef.nResults != op->getNumResults()) {
        // llvm::outs() << "!! Result mismatch: MLIR has " << op->getNumResults()
        //              << ", Egglog expects " << egglogOpDef.nResults << "\n";
        return eggifyOpaqueOperation(op);
    }
    ...
    // ==== Operands ====
    // llvm::outs() << "Eggifying operands...\n";
    std::vector<EggifiedOp*> operands;
    for (size_t i = 0; i < egglogOpDef.nOperands; i++) {
        // llvm::outs() << "  Operand[" << i << "] -> " << op->getOperand(i) << "\n";
        EggifiedOp* eggifiedOperand = eggifyValue(op->getOperand(i));
        operands.push_back(eggifiedOperand);
        ss << " " << eggifiedOperand->getPrintId();
    }
    ...
    // ==== Regions ====
    for (size_t i = 0; i < egglogOpDef.nRegions; i++) {
        // llvm::outs() << "Eggifying region " << i << "...\n";
        ss << " " << eggifyRegion(op->getRegion(i));
    }

    // ==== Types ====
    for (size_t i = 0; i < egglogOpDef.nResults; i++) {
        // llvm::outs() << "Eggifying result type " << i << "...\n";
        ss << " " << eggifyType(op->getResult(i).getType());
    }

    ss << ")";

    // llvm::outs() << "Final Egglog expression: " << ss.str() << "\n";
    ...
    eggifiedBlock.push_back(eggifiedOp);
    // llvm::outs() << "=== Finished eggifying op ===\n";
    return eggifiedOp;
}
