...
//   vdsa_output/dpa_handler.c   — DPA block functions (FlexIO restricted C)
//   vdsa_output/arm_handler.ll  — ARM block functions as LLVM IR (.ll)
//   vdsa_output/Makefile        — compiles arm_handler.ll + test harness
//
// ARM blocks are lowered from vdpp.mlir → LLVM IR via VDPPToLLVM.
// DPA blocks are lowered from bf3dpa.mlir → restricted C via BF3DPAToDPACode.
// ============================================================================
...
static LogicalResult generateMakefile(llvm::StringRef inputDir,
    ...
    out << "# Makefile — generated by nutcracker (do not edit)\n"
        << "# arm_handler.ll is LLVM IR; compile with clang for your ARM target.\n"
        << "CC      ?= clang\n"
        << "CFLAGS  ?= -O2 -I.\n\n";

    // ARM test binary: compile arm_handler.ll + test main.
    out << "arm_handler.o: arm_handler.ll\n"
        << "\t$(CC) $(CFLAGS) -c arm_handler.ll -o arm_handler.o\n\n"
        << "all: arm_test\n\n"
        << "arm_test: arm_handler.o arm_test_main.c nc_types.h\n"
        << "\t$(CC) $(CFLAGS) arm_handler.o arm_test_main.c -o arm_test\n\n";
    ...
    out << "clean:\n\trm -f arm_test arm_handler.o arm_test_main.c\n\n"
        << ".PHONY: all clean\n";

    // Generate test main (C, links against the compiled arm_handler.o)
    std::string testFile = (inputDir + "/arm_test_main.c").str();
    llvm::raw_fd_ostream test(testFile, EC);
    if (EC) { llvm::errs() << "  ✗ Cannot open " << testFile << "\n"; return failure(); }

    // Forward-declare the ARM block functions from arm_handler.ll
    test << "/* arm_test_main.c — generated by nutcracker (do not edit) */\n"
         << "#include <stdio.h>\n"
         << "#include <string.h>\n"
         << "#include \"nc_types.h\"\n\n";
    for (int id : armBlocks)
        test << "int nc_block" << id << "_arm(void *, void *, void *);\n";
    test << "\n"
         << "int main(void) {\n"
...
public:
    ...
    llvm::StringRef getDescription() const override {
        return "Emit nc_types.h, dpa_handler.c, arm_handler.ll from mapping";
    }
    ...
    void runOnOperation() override {
        ...
        if (dpaBlocks.empty() && armBlocks.empty()) {
            ...
            // Step 4: ARM handler (LLVM IR) + Makefile
            if (!armBlocks.empty()) {
                llvm::outs() << "  Generating arm_handler.ll ("
                             << armBlocks.size() << " blocks) ...\n";
                if (failed(emitARMHandler(inputDir, armBlocks))) {
                    signalPassFailure(); return;
                }
                llvm::outs() << "  ✓ Generated arm_handler.ll\n";
        ...
        // Step 5: Always generate the codegen/ directory with the full mapping
        llvm::outs() << "\n  Generating codegen/ directory ...\n";
        if (failed(generateDeployDir(inputDir, mapping, dpaBlocks, armBlocks,
                                     drmtBlocks))) {
            signalPassFailure(); return;
        }
        llvm::outs() << "  ✓ Generated codegen/\n";
...
private:
    ...
    // =========================================================================
    // generateDeployDir — produce vdsa_output/codegen/ runtime integration files
    // =========================================================================
    LogicalResult generateDeployDir(llvm::StringRef inputDir,
                                     ...
                                     const std::vector<int> &armBlocks,
                                     const std::vector<int> &drmtBlocks) {
        std::string deployDir = (inputDir + "/codegen").str();
        ...
        // ── 2. Copy nc_types.h from vdsa_output/ ─────────────────────────────
        {
            ...
            if (bufOrErr) {
                ...
                // a minimal stub so codegen/ still compiles.
                std::error_code EC;
                llvm::raw_fd_ostream out(destFile, EC);
                if (!EC) {
        ...
        // ── 5b. codegen/nc_dpa_stubs.c (no-DPA case) ────────────────────────
        ...
        // ── 6. codegen/meson.build ───────────────────────────────────────────
        {
            ...
            // codegen/ is first in include_directories so the generated nc_pipeline.h
        ...
        // ── 7. codegen/doca_flow_pipeline.c ──────────────────────────────────
        ...
        {
            llvm::outs() << "  Generating codegen/doca_flow_pipeline.c"
                         << " (with HW mapping) ...\n";
        ...
        llvm::outs() << "    codegen/nc_pipeline.h  (" << numBlocks << " blocks, "
                     << armEntrypoints.size() << " ARM queues)\n"
                     << "    codegen/nc_types.h\n"
                     << "    codegen/doca_flow_pipeline.c\n";
        if (!armBlocks.empty()) llvm::outs() << "    codegen/arm_handler.ll\n";
        if (!dpaBlocks.empty()) {
            llvm::outs() << "    codegen/dpa_handler.c\n"
                         << "    codegen/dpa_dev_entry.c\n";
        } else {
            llvm::outs() << "    codegen/nc_dpa_stubs.c\n";
        }
        llvm::outs() << "    codegen/meson.build\n";
    ...
    LogicalResult emitARMHandler(llvm::StringRef inputDir,
                                  const std::vector<int> &blocks) {
        // arm.ll was already generated per-block by VDPPToBF3DPAPass.
        // Here we just concatenate the pre-generated blockN/arm.ll files into
        // a single arm_handler.ll for the deploy build.
        std::string outFile = (inputDir + "/arm_handler.ll").str();
        std::error_code EC;
        llvm::raw_fd_ostream out(outFile, EC);
        if (EC) { llvm::errs() << "  ✗ Cannot open " << outFile << "\n"; return failure(); }

        out << "; arm_handler.ll — generated by nutcracker (do not edit)\n"
            << "; Compile with: clang -O2 -c arm_handler.ll -o arm_handler.o\n\n";
        ...
        for (int blockId : blocks) {
            std::string armLL = (inputDir + "/block" + std::to_string(blockId)
                                 + "/arm.ll").str();
            auto buf = llvm::MemoryBuffer::getFile(armLL);
            if (!buf) {
                llvm::errs() << "  ✗ Cannot read " << armLL
                             << " (was VDPPToBF3DPAPass run?)\n";
                return failure();
            }
            out << (*buf)->getBuffer() << "\n";
        }
