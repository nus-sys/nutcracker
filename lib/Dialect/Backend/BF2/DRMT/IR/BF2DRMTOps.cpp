//===- BF2DRMTOps.cpp - BF3 NIC ASIC DRMT dialect ops --------*- C++ -*-===//
//===----------------------------------------------------------------------===//
...
#include "Dialect/Backend/BF2/DRMT/IR/BF2DRMTDialect.cpp.inc"
#include "Dialect/Backend/BF2/DRMT/IR/BF2DRMTOpsEnums.cpp.inc"
...
mlir::LogicalResult ReadOp::verify() {
    ...
    if (!mlir::isa<StructExtractRefOp>(defOp) &&
        ...
        return emitOpError(
            "ref operand must be produced by 'bf2drmt.struct_extract_ref' or "
            "'bf2drmt.variable' (BF3 NIC ASIC constraint: struct_extract_ref -> "
...
mlir::LogicalResult StructExtractRefOp::verify() {
    ...
    if (!mlir::isa<mlir::vdrmt::ReferenceType>(inputTy) &&
        !mlir::isa<mlir::bf2drmt::ReferenceType>(inputTy)) {
        return emitOpError("input must be a !vdrmt.ref<...> or !bf2drmt.ref<...> "
                           "reference type, got '")
               << inputTy << "'";
    }
    auto resultTy = getResult().getType();
    if (!mlir::isa<mlir::vdrmt::ReferenceType>(resultTy) &&
        !mlir::isa<mlir::bf2drmt::ReferenceType>(resultTy)) {
        return emitOpError("result must be a !vdrmt.ref<...> or !bf2drmt.ref<...> "
                           "reference type, got '")
               << resultTy << "'";
    }
    return mlir::success();
...
mlir::LogicalResult BinOp::verify() {
    if (getKind() == BF2DRMTBinOpKind::Sub) {
...
// TableKeyOp
//
// Verifier: the body must end with a bf2drmt.match_key op.
//===----------------------------------------------------------------------===//
...
mlir::LogicalResult TableKeyOp::verify() {
    ...
    if (lastBlock.empty() || !mlir::isa<MatchKeyOp>(lastBlock.back()))
        return emitOpError(
            "table_key body must end with a 'bf2drmt.match_key' op");
    return mlir::success();
}
...
mlir::LogicalResult PipeOp::verify() {
    ...
    if (getMatchMask().has_value()) {
        if (ptype != BF2DRMTPipeType::Basic && ptype != BF2DRMTPipeType::Acl) {
            return emitOpError(
                "match_mask is only valid for 'basic' and 'acl' pipe types");
        }
    }
    // nr_entries required for hash pipe.
    if (ptype == BF2DRMTPipeType::Hash && !getNrEntries().has_value()) {
...
// PipeActionOp
//
// The body must end with a bf2drmt.next op (the hit successor). Forward
...
mlir::LogicalResult PipeActionOp::verify() {
    ...
    if (lastBlock.empty() || !mlir::isa<NextOp>(lastBlock.back())) {
        return emitOpError(
            "pipe_action body must end with a 'bf2drmt.next' op specifying "
