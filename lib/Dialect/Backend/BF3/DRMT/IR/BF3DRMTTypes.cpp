//===- BF3DRMTTypes.cpp - BF3 NIC ASIC DRMT dialect types ------*- C++ -*-===//
//===----------------------------------------------------------------------===//
...
#include "Dialect/Backend/BF3/DRMT/IR/BF3DRMTTypes.cpp.inc"

// ── Dialect-level parse/print hooks ──────────────────────────────────────────

mlir::Type mlir::bf3drmt::BF3DRMTDialect::parseType(
    ...
    parser.emitError(typeLoc) << "unknown bf3drmt type: " << mnemonic;
    return mlir::Type();
}

void mlir::bf3drmt::BF3DRMTDialect::printType(
        mlir::Type type, mlir::DialectAsmPrinter &os) const {
    if (generatedTypePrinter(type, os).succeeded()) return;
    llvm::report_fatal_error("bf3drmt: missing type printer");
}
...
void mlir::bf3drmt::BF3DRMTDialect::registerTypes() {
    addTypes<
